...
export function OnboardingModal({ visible, onComplete, onSkip, userId }: OnboardingModalProps) {
  ...
  useEffect(() => {
    if (visible && currentStep === 1 && availableGenres.length === 0) {
      fetchGenres();
    }
  }, [visible, currentStep]);
  ...
  const saveGenrePreferences = async () => {
    ...
    try {
      // Here we would make an API call to save the user's genre preferences
      // This endpoint would need to be implemented on the backend
      await apiRequest("POST", `/api/users/${userId}/preferences/genres`, {
