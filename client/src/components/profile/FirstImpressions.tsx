...
export default function FirstImpressions({ watchedMovies, isLoading }: FirstImpressionsProps) {
  ...
  if (moviesWithImpressions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-40 text-gray-500">
        <QuoteIcon className="h-10 w-10 mb-2 text-gray-300" />
  ...
  return (
    <div className="space-y-4 p-4">
      {moviesWithImpressions.map(movie => (
        <div 
          key={movie.id} 
          className="bg-amber-50 border border-amber-100 rounded-lg overflow-hidden"
        >
          <div className="flex items-center p-3 bg-amber-100/50 border-b border-amber-100">
            <img 
          ...
          <div className="p-4">
            <div className="flex">
              <QuoteIcon className="h-4 w-4 text-amber-800 mr-2 flex-shrink-0 mt-1" />
